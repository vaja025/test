...
Friend Class LodgeItem
    ...
    Public Function Validate(ByVal Name As String, ByVal Number As String, ByVal CheckOnly As Boolean) As Boolean

        Dim items As List(Of Lodge) = Search(Name, Number, Province)

        Using objForm As New LodgeSelection
            ...
            ElseIf Not CheckOnly Then
                ...
                If items.Count.Equals(0) Then
                    ' Widen the search
                    items = Search(Name, Number, String.Empty, False)
                End If

                objForm.LodgeName = Name
                objForm.Number = Number
                objForm.Province = Province
    ...
    Friend Shared Function Search(ByVal Name As String,
                                  ...
                                  Optional ByVal Exact As Boolean = True) As List(Of Lodge)

        Dim results As New List(Of Lodge)
        ...
        Dim requestString As String = String.Format(New Abbyy().OracleLookupBaseUrl + "Lodge?apikey={0}&name={1}&number={2}&province={3}&exact={4}&xml=True", New Abbyy().OracleLookupApiKey, _name, _number, _province, Exact)
        ...
        Dim serializer As New XmlSerializer(GetType(List(Of Lodge)))
